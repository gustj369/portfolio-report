...
from limiter import limiter  # 순환 import 방지 — main.py와 동일 인스턴스 공유
...
async def analyze_portfolio(
    ...
    try:
        ...
        market_snapshot = await _loop.run_in_executor(None, fetch_market_snapshot_cached, settings.fred_api_key)
        t1 = time.perf_counter()
        logger.info(f"시장 데이터 수집 완료 ({t1 - t0:.2f}s 누적)")

        # 2. 5년 시뮬레이션
        simulation = run_simulation(body.portfolio, market_snapshot)
        t2 = time.perf_counter()
        logger.info(f"시뮬레이션 완료 ({t2 - t1:.2f}s 스테이지, {t2 - t0:.2f}s 누적)")

        # 3. 리스크 점수 계산
        risk_score, risk_grade = calculate_risk_score(body.portfolio, market_snapshot)
        logger.info(f"리스크 점수: {risk_score} ({risk_grade})")

        # 4. AI 간략 요약 생성
        # Gemini 키가 있으면 AI 분석을 시도하고, 없거나 실패한 경우에만 fallback 호출
        ...
        if settings.gemini_api_key:
            try:
                ...
                summary, ai_risk_score, ai_risk_grade = await _loop.run_in_executor(
                    ...
                    body.user_profile, body.portfolio,
                ...
                summary = generate_personalized_preview_summary(
                    body.user_profile, body.portfolio, market_snapshot, risk_score, risk_grade, simulation
                )
        else:
            elapsed = time.perf_counter()
